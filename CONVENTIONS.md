...
- Test you API via the documentation - Open http://localhost/rewards/api/doc/ui/ , choose "Local server" from the "Servers" combobox and run your API. This is an alternative to Postman. Make sure to set the correct value in the `HOST` environment variable (eg. http://localhost/rewards/api) before running the service to access the docs.
